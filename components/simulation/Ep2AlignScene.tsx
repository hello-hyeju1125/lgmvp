...
export function Ep2AlignScene({ userName: _userName }: Ep2AlignSceneProps) {
  ...
  /* Chain states:
     title(0), situation(1), narration-d0(2), 김지훈-bubble(3) [stagger reveal]
     → typing done → narration-d2 fade → 박소진-bubble fade → typing done
     → action-narration fade → Q → options */
  const [firstTypingDone, setFirstTypingDone] = useState(false);
  const [showNarration2, setShowNarration2] = useState(false);
  const [showSecondSpeaker, setShowSecondSpeaker] = useState(false);
  const [secondSpeakerUnlocked, setSecondSpeakerUnlocked] = useState(false);
  const [secondTypingDone, setSecondTypingDone] = useState(false);
  const [showActionNarration, setShowActionNarration] = useState(false);
  ...
  const handleNarration2FadeEnd = useCallback(
    (e: AnimationEvent<HTMLDivElement>) => {
      if (reducedMotion) return;
      if (!isSeqFadeAnimation(e)) return;
      setShowSecondSpeaker(true);
    },
    [reducedMotion],
  );
  ...
  const handleActionFadeEnd = useCallback(
    (e: AnimationEvent<HTMLDivElement>) => {
      if (reducedMotion) return;
      if (!isSeqFadeAnimation(e)) return;
      if (showQuestionBlock) setShowQ(true);
      else setOptionsVisibleCount(1);
    },
    [reducedMotion, showQuestionBlock],
  );
  ...
  const ep2OptionCount = ep2AlignOptions.length;

  const handleOptionFadeEnd = useCallback(
    (optIdx: number) => (e: AnimationEvent<HTMLDivElement>) => {
      if (reducedMotion) return;
      if (!isSeqFadeAnimation(e)) return;
      if (optIdx < ep2OptionCount - 1) setOptionsVisibleCount(optIdx + 2);
    },
    [reducedMotion, ep2OptionCount],
  );
  ...
  useLayoutEffect(() => {
    ...
    setSecondSpeakerUnlocked(true);
    setSecondTypingDone(true);
    setShowActionNarration(true);
    setShowQ(true);
    setShowQuestionWording(true);
    setOptionsVisibleCount(ep2OptionCount);
  }, [reducedMotion, ep2OptionCount]);
  ...
  useEffect(() => {
    ...
    setShowQuestionWording(true);
    setOptionsVisibleCount(ep2OptionCount);
  }, [reducedMotion, secondTypingDone, ep2OptionCount]);
    ...
    <section className="ep1-scene-layout w-full min-w-0 max-w-none space-y-10 sm:space-y-12">
      ...
      <div className="space-y-2 px-2 text-center">
        <p
          ...
          style={revealDelay(1)}
        >
          {renderDialogueBold(ep2AlignScene.situation, ACCENT)}
        </p>
      </div>
      ...
      <div className="space-y-6 sm:space-y-8">
        ...
        >
          <p className="font-sans text-[19px] font-medium leading-relaxed text-[#111] sm:text-[21px]">
            {renderDialogueBold(d0, ACCENT)}
          </p>
        </div>

        {/* d1: 김지훈 speaker with typing */}
        <div className="ep1-scene-reveal" style={revealDelay(3)}>
          <Ep2SpeakerBubble
            name="김지훈 선임 (IT)"
        ...
        {/* d2: narration (after first typing) */}
        {firstTypingDone && (
          <div
            className="ep1-dialogue-seq-fade rounded-xl bg-[#eceeef] px-5 py-5 text-center sm:px-7 sm:py-6"
            onAnimationEnd={handleNarration2FadeEnd}
          >
            <p className="font-sans text-[19px] font-medium leading-relaxed text-[#111] sm:text-[21px]">
              {renderDialogueBold(d2, ACCENT)}
            </p>
          </div>
        )}

        {/* d3: 박소진 speaker with typing */}
        {showSecondSpeaker && (
          <div className="ep1-dialogue-seq-fade" onAnimationEnd={handleSecondShellFadeEnd}>
            <Ep2SpeakerBubble
              ...
              canStart={secondSpeakerUnlocked}
      ...
      {/* Action narration + Q + options (after second typing) */}
      {secondTypingDone && (
        <>
          <div className="space-y-10 sm:space-y-12">
            ...
            >
              <p className="font-sans text-[19px] font-medium leading-relaxed text-[#111] sm:text-[21px]">
                {renderDialogueBold(showQuestionBlock ? actionLead : actionFull, ACCENT)}
              </p>
            </div>
          </div>

          {showQuestionBlock && showQ && (
            <div className="space-y-4 px-1 pt-4 text-center !mt-20 sm:!mt-24 mb-[3.75rem] sm:mb-[4.5rem]">
              <p
                className="ep1-dialogue-seq-fade m-0 font-sans text-[56px] font-black leading-none text-black sm:text-[72px]"
              ...
              {showQuestionWording && (
                <p
                  className="ep1-dialogue-seq-fade m-0 mx-auto max-w-[min(100%,40rem)] font-sans text-[19px] font-medium leading-relaxed text-[#111] sm:text-[21px]"
