...
function SummaryItem({ title, body, isLast, step }: { title: string; body: string; isLast: boolean; step: number }) {
  return (
    <div className="ep1-scene-reveal space-y-2 pb-6 sm:pb-7" style={revealDelay(step)}>
      <div className="flex items-start gap-2.5">
        <span className="mt-[3px] flex h-5 w-5 shrink-0 items-center justify-center text-[#059669]">
  ...
  return (
    ...
    >
      <div className="flex w-[128px] shrink-0 items-center gap-2 sm:w-[148px]">
        ...
        <span className="text-[14px] font-extrabold leading-tight text-[#333] sm:text-[16px]">{label}</span>
      </div>
      <div className="relative h-[14px] min-w-0 flex-1 overflow-hidden rounded-full bg-[#e5e7eb]">
        <div
          className="absolute inset-y-0 left-0 rounded-full bg-[#64e87a]"
          style={{
            width: `${animatedWidth}%`,
      ...
      <span className="min-w-[3rem] shrink-0 whitespace-nowrap text-right text-[14px] font-extrabold tabular-nums leading-none text-[#22c55e] sm:min-w-[3.25rem] sm:text-[16px]">
        {displayPct}%
      </span>
      <span
        className={`recap-kpi-delta w-[40px] shrink-0 whitespace-nowrap text-right text-[11px] font-bold tabular-nums sm:w-[46px] sm:text-[12px] ${
          deltaLabel
            ? went_up
              ? "recap-kpi-delta--up"
...
export function InitiationRecap({ userName }: InitiationRecapProps) {
  ...
  const summaryParsed = initiationRecapCopy.summaryItems.map((item) => {
    ...
    if (colonIdx === -1) return { title: item, body: "" };
    return { title: item.slice(0, colonIdx).trim(), body: item.slice(colonIdx + 1).trim() };
  });
  ...
  /** 카드가 보인 뒤 막대별 시작 간격 (한 줄씩 느리게 이어짐) */
    ...
    <section className="ep1-scene-layout w-full min-w-0 max-w-none">
      ...
      <div className="mb-10 space-y-1 px-2 text-center sm:mb-12">
        ...
        <p className="ep1-scene-reveal m-0 text-[17px] font-medium leading-[2] text-[#374151] sm:text-[18px]" style={revealDelay(3)}>
          현재 <strong className="font-bold text-[#64e87a]">프로젝트의 기초</strong>가 만들어졌습니다.
        </p>
      </div>
      ...
      >
        ...
        {initiationRecapCopy.summaryEpigraph ? (
          <p
            className="ep1-scene-reveal mx-6 mb-5 border-l-4 border-[#64e87a] bg-[#f0fdf4]/90 px-4 py-3 text-left text-[15px] font-semibold leading-relaxed text-[#14532d] sm:mx-8 sm:px-5 sm:text-[16px]"
            style={revealDelay(5)}
          >
            {initiationRecapCopy.summaryEpigraph}
          </p>
        ...
        <div className="px-6 pt-2 sm:px-8">
          {summaryParsed.map((item, i) => (
            <SummaryItem key={i} title={item.title} body={item.body} isLast={i === summaryParsed.length - 1} step={6 + i} />
          ))}
        </div>
      </div>
      ...
      >
        ...
        <div className="flex flex-col gap-6 px-6 pb-8 pt-4 sm:px-8 sm:pb-10 md:flex-row md:gap-0">
          {/* 좌측 (~45%): 의사결정 내역 */}
          <div className="min-w-0 space-y-5 md:w-[45%] md:shrink-0 md:border-r md:border-black/10 md:pr-8">
            {/* 액션 아이템 선택 */}
              ...
              <div className="flex flex-col gap-2">
                {selectedActions.length > 0 ? (
                  selectedActions.map((a) => (
                    <span
                      key={a.id}
                      className="recap-green-chip flex w-full items-center gap-2 rounded-none bg-[#4ade80] px-4 py-2.5 text-[13px] font-bold text-white sm:text-[14px]"
                    >
            ...
            <div className="ep1-scene-reveal space-y-2" style={revealDelay(11)}>
              <p className="text-[14px] font-extrabold text-[#111] sm:text-[15px]">E1. 위에서 떨어진 폭탄</p>
              {e1Label ? (
            ...
            <div className="ep1-scene-reveal space-y-2" style={revealDelay(12)}>
              <p className="text-[14px] font-extrabold text-[#111] sm:text-[15px]">E2. 유관부서와의 동상이몽</p>
              {e2Label ? (
                <span className="recap-green-chip flex w-full items-center gap-2 rounded-none bg-[#4ade80] px-4 py-2.5 text-[13px] font-bold text-white sm:text-[14px]">
                  <span className="flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-white">
                    <svg className="h-3 w-3 text-[#4ade80]" viewBox="0 0 12 10" fill="none" aria-hidden>
          ...
          {/* 우측 (~55%): KPI 프로그레스 바 */}
          <div className="flex min-w-0 flex-col justify-between md:w-[55%] md:pb-1 md:pl-8 md:pt-8">
      ...
      <div className="mt-16 space-y-1 px-2 text-center sm:mt-20">
        ...
        <p className="ep1-scene-reveal m-0 text-[17px] font-medium leading-[2] text-[#374151] sm:text-[18px]" style={revealDelay(15)}>
          다음 단계로 넘어가면 <strong className="font-bold text-[#059669]">선배 PM 노하우</strong>가 이어집니다.
        </p>
      </div>
    </section>
  );
