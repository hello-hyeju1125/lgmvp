      ...
      <div className={`d1-kpi-bar-track relative h-[14px] flex-1 overflow-hidden rounded-full bg-[#e5e7eb] ${critical ? "kpi-critical-track" : ""}`}>
        <div
          ...
          style={{
            ...
            ...(!critical ? { backgroundColor: went_down && transitioning ? "#ef4444" : "#FF7A00" } : {}),
          }}
        />
      </div>
      <span
        ...
        style={!critical ? { color: went_down && transitioning ? "#ef4444" : "#FF7A00" } : {}}
      >
        {displayPct}%
      </span>
      <span
...
export function ExecD1Popup({ userName: _userName }: ExecD1PopupProps) {
  ...
  const [capturedBefore] = useState<KpiState>(() =>
    kpiBeforeExecution ? { ...kpiBeforeExecution } : { ...kpi },
  );
  ...
  return (
    ...
    >
      <div className="initiation-scenario-block mb-8 text-center sm:mb-10">
        ...
        >
          {renderGreenEmphasis(executionScreenCopy.popupIntro)}
        </p>
      </div>
        ...
        >
          <div className="action-card-body w-full min-w-0">
            <span className="impact-tag inline-flex w-fit font-sans">
              {executionScreenCopy.popupBadgesLabel.replace(/:\s*$/, "")}
            </span>
            <div className="mt-4 flex flex-wrap gap-2.5">
              {badges.length > 0 ? (
                badges.map((b) => (
                  <span
                    key={b}
                    className="initiation-d1-chip initiation-footer-chip-green inline-flex max-w-[min(100%,240px)] items-center gap-1.5 rounded-[10px] border-2 border-black bg-[#FF7A00] px-3 py-2 font-sans text-[12px] font-extrabold text-black shadow-[2px_2px_0_#111111] sm:text-[13px]"
                  >
      ...
      <div ref={kpiCardRef} className="mb-9 flex w-full justify-center sm:mb-10">
        ...
        >
          <div className="action-card-body w-full min-w-0">
            <span className="impact-tag inline-flex w-fit font-sans">{executionScreenCopy.popupKpiLabel}</span>
