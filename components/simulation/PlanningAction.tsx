...
export function PlanningAction({ userName, stage = "alloc" }: PlanningActionProps) {
  ...
  const progressPct = Math.min(100, Math.max(0, (total / PLANNING_TOTAL_HOURS) * 100));
  const isOver = total > PLANNING_TOTAL_HOURS;
    ...
    <div className="mx-auto w-full max-w-4xl space-y-6 px-6 py-6">
      {stage === "intro" && (
        <section className="rounded-[0.735rem] border border-white/15 bg-white/95 p-6 shadow-[0_24px_90px_rgba(0,0,0,0.55)] backdrop-blur-md sm:p-8">
          ...
          <p className="mt-4 text-[15px] leading-[1.85] text-black/75">{renderWithBold(planningScreenCopy.intro1)}</p>
          <p className="mt-3 text-[15px] leading-[1.85] text-black/75">{renderWithBold(planningScreenCopy.intro2)}</p>
          ...
          <div className="mt-6 overflow-hidden rounded-[0.49rem] border border-black/10 bg-[#f8f9fa] shadow-[0_12px_40px_rgba(0,0,0,0.08)]">
            <div className="flex items-center gap-3 border-b border-black/10 bg-white/60 px-5 py-3">
              ...
              <div className="min-w-0">
                ...
                <p className="mt-0.5 text-[13px] font-extrabold text-black/85">계획 품질을 높이는 핵심은 무엇일까요?</p>
              </div>
            </div>
            <div className="border-l-4 border-[#E4003F] px-5 py-4">
              <p className="text-[14px] leading-[1.85] text-black/75">
                계획 단계에서는 일정, 리소스, 리스크, 커뮤니케이션 원칙을 선제적으로 설계할수록
                실행 단계의 혼선과 재작업을 줄일 수 있습니다.
              </p>
            </div>
          </div>
        </section>
        ...
        <>
            ...
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <div className="min-w-0">
                ...
                <p className={`mt-1 text-[16px] font-extrabold ${isValid ? "text-[#E4003F]" : isOver ? "text-[#E4003F]" : "text-black/85"}`}>
                  {total}시간 / {PLANNING_TOTAL_HOURS}시간
          ...
          <div className="space-y-4">
            {planningActions.map((action, idx) => {
              ...
              return (
                <div key={action.id} className="rounded-2xl border border-white/15 bg-white/95 p-5 shadow-[0_10px_30px_rgba(0,0,0,0.06)] backdrop-blur-md">
