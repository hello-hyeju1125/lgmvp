...
function Ep4SpeakerBubble({
  ...
  return (
    <div className="flex items-start gap-0">
      <div className="ep1-scene-reveal relative z-30 shrink-0" style={revealDelay(avatarStep)}>
      ...
      <div
        className="ep1-scene-reveal relative z-10 min-w-0 flex-1"
        style={revealDelay(bubbleStep)}
      >
...
export function Ep4RoleScene({ userName: _userName }: Ep4RoleSceneProps) {
  ...
  const ep4OptionCount = ep4Options.length;

  const handleOptionFadeEnd = useCallback(
    (optIdx: number) => (e: AnimationEvent<HTMLDivElement>) => {
      if (reducedMotion) return;
      if (!isSeqFadeAnimation(e)) return;
      if (optIdx < ep4OptionCount - 1) setOptionsVisibleCount(optIdx + 2);
    },
    [reducedMotion, ep4OptionCount],
  );
  ...
  useLayoutEffect(() => {
    ...
    setShowPostNarration(true);
    setShowQuestionWording(true);
    setOptionsVisibleCount(ep4OptionCount);
  }, [reducedMotion, ep4OptionCount]);
    ...
    <section className="ep1-scene-layout w-full min-w-0 max-w-none space-y-10 sm:space-y-12">
      ...
      <div className="space-y-2 px-2 text-center">
        <p
          ...
          style={revealDelay(1)}
        >
          {renderDialogueBold(ep4Scene.situation, ACCENT)}
        </p>
      </div>
        ...
        {typingDone && (
          ...
          >
            <p className="whitespace-pre-line font-sans text-[19px] font-medium leading-relaxed text-[#111] sm:text-[21px]">
              {renderDialogueBold(ep4Scene.dialogue[1], ACCENT)}
            </p>
          </div>
        )}
      </div>
            ...
            <div className="space-y-4 px-1 pt-4 text-center !mt-20 sm:!mt-24 mb-[3.75rem] sm:mb-[4.5rem]">
              ...
              {showQuestionWording && (
                ...
                >
                  ...
                  <p>
                    <span className="font-bold" style={{ color: ACCENT }}>{EP4_ACTION_QUESTION}</span>
                  </p>
                </div>
              )}
            </div>
