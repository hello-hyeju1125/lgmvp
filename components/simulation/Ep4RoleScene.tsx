...
export function Ep4RoleScene({ userName }: Ep4RoleSceneProps) {
  ...
  return (
    <div className="mx-auto w-full max-w-3xl space-y-6">
      <h2 className="text-2xl sm:text-3xl font-extrabold tracking-tight text-black/90">{ep4Scene.title}</h2>
      <p className="text-[18px] font-extrabold leading-[1.85] text-black/90">
        <span className="text-black/90">[Situation]</span> {renderWithBold(ep4Scene.situation)}
      </p>
                ...
                <div className="flex items-center justify-between gap-3">
                  <div className="min-w-0">
                    ...
                    <p className="mt-1 line-clamp-2 text-[14px] leading-[1.75] text-black/40 blur-[0.6px]">
                      {renderWithBold(ep4Scene.dialogue[0] ?? "")}
                    </p>
                  </div>
      ...
      <p className="text-[18px] font-extrabold leading-[1.85] text-black/90">
        <span className="text-black/90">[Action]</span> {renderWithBold(ep4Scene.action)}
      </p>
