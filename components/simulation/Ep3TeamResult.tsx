...
const ALL_CHOICES: Ep3Choice[] = ["A", "B", "C", "D"];
...
function FeedbackPanel({ paragraphs, embedded = false }: { paragraphs: string[]; embedded?: boolean }) {
  return (
    <div
      className={
        embedded
          ? "ep1-result-feedback-panel planning-ep-feedback-panel border-0 bg-[#F7FBFF] px-6 pb-8 pt-7 sm:px-8 sm:pb-9 sm:pt-8"
          : "ep1-result-feedback-panel planning-ep-feedback-panel mt-6 rounded-none border-0 bg-[#F7FBFF] px-6 py-6 pt-7 sm:px-8"
      }
    >
...
export function Ep3TeamResult({ userName: _userName }: Ep3TeamResultProps) {
  const router = useRouter();
  const { episode3Choice, planningActionHours } = useStore();
  const [showOthers, setShowOthers] = useState(false);

  const choice = episode3Choice as Ep3Choice | null;
  const valid = choice === "A" || choice === "B" || choice === "C" || choice === "D";
    ...
    return (
      ...
      >
        ...
        <button
          ...
          className="mt-4 rounded-xl border-2 border-black bg-[color:var(--sim-accent)] px-5 py-2.5 text-sm font-bold text-[color:var(--sim-accent-cta-text)]"
        >
          옵션 선택으로 돌아가기
        </button>
      </section>
  ...
  const selected = getEp3Result(choice, hours);
  const others = ALL_CHOICES.filter((id) => id !== choice);
  const episodeTitle = ep3Scene.title;
      ...
      <div className="mx-auto w-full max-w-7xl">
        ...
        <div className="ep1-result-card-root relative z-[1] mt-8 rounded-none border-[3px] border-black bg-white sm:mt-10">
          <div className="ep1-scene-reveal relative" style={planningRevealDelay(2)}>
            <div
              className="ep1-result-my-choice-chip absolute left-4 top-0 z-20 flex -translate-y-1/2 items-center gap-2 rounded-none border-0 bg-[color:var(--sim-accent)] px-4 py-2.5 text-[16px] font-extrabold text-[color:var(--sim-accent-cta-text)] sm:left-7 sm:text-[17px]"
              role="status"
              aria-label="나의 선택"
            >
              <span className="flex h-6 w-6 items-center justify-center rounded-full border-0 bg-white">
                <svg width="14" height="11" viewBox="0 0 12 10" fill="none" aria-hidden>
                  <path d="M1 5L4 8L11 1" stroke="var(--sim-accent)" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" />
                </svg>
              </span>
              나의 선택
            </div>
