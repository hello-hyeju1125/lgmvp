...
export function PlanRecap({ userName }: PlanRecapProps) {
  const {
    kpi, kpiStartPlanning, planningActionHours, initiationActionHours,
    episode3Choice, episode4Choice, episode5Choice,
    kpiBeforeEp3Result, kpiBeforeEp4Result, kpiBeforeEp5Result,
  } = useStore();
  ...
  const stepSnapshots = useMemo(() => {
    const base = kpiStartPlanning ?? kpi;
    ...
    const afterPlan = kpiBeforeEp3Result
      ?? simulateApplyDelta(base, getPlanningKpiDelta(planningActionHours));

    const ep3Delta = episode3Choice
      ? getEp3Result(episode3Choice, planningActionHours["resource_assign"] ?? 0)?.kpi
      : null;
    const afterEp3 = kpiBeforeEp4Result
      ?? (ep3Delta ? simulateApplyDelta(afterPlan, ep3Delta) : afterPlan);

    const ep4Delta = episode4Choice
      ? getEp4Result(episode4Choice, initiationActionHours["team_profile"] ?? 0)?.kpi
      : null;
    const afterEp4 = kpiBeforeEp5Result
      ?? (ep4Delta ? simulateApplyDelta(afterEp3, ep4Delta) : afterEp3);
    ...
    return [
      ...
      { label: "액션 아이템", sub: null, values: afterPlan, deltas: computeDelta(afterPlan, base) },
      { label: "E3 결과", sub: e3Label, values: afterEp3, deltas: computeDelta(afterEp3, afterPlan) },
      { label: "E4 결과", sub: e4Label, values: afterEp4, deltas: computeDelta(afterEp4, afterEp3) },
      { label: "E5 결과", sub: e5Label, values: kpi, deltas: computeDelta(kpi, base) },
    ] as Row[];
  }, [kpiStartPlanning, kpiBeforeEp3Result, kpiBeforeEp4Result, kpiBeforeEp5Result, kpi, selectedActions, e3Label, e4Label, e5Label, planningActionHours, initiationActionHours, episode3Choice, episode4Choice]);
