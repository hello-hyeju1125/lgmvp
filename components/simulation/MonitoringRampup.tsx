...
const RED_SOFT_BG = "rgba(239, 68, 68, 0.08)";
const RED_CARD_BG = "#fef2f2";
        ...
        <div
          className={`relative z-10 flex h-10 w-10 shrink-0 items-center justify-center rounded-full border-2 sm:h-12 sm:w-12 ${
            isDone
              ...
              : isNext
                ? "border-[#ef4444] bg-white mon-rampup-dot-pulse"
                : "border-[#d1d5db] bg-white"
          }`}
        >
          {isDone ? (
            <CheckCircle2 className="mon-rampup-done-icon h-5 w-5 text-white sm:h-6 sm:w-6" strokeWidth={2.5} />
          ) : (
            <Icon
              className={`h-5 w-5 sm:h-6 sm:w-6 ${isNext ? "text-[#ef4444]" : "text-[#9ca3af]"}`}
            />
          )}
        </div>
        {!isLast && (
      ...
      <div
        className={`mb-4 min-w-0 flex-1 rounded-xl border-2 p-5 transition-all sm:mb-5 sm:p-6 ${
          isDone
            ...
            : isNext
              ? "mon-rampup-next-card border-[#ef4444] bg-white shadow-[0_0_24px_rgba(239,68,68,0.12)]"
              : "border-[#e5e7eb] bg-[#fafafa]"
        }`}
      >
        <div className="mb-2 flex items-center gap-2">
          {isDone && (
            <span className="mon-rampup-done-badge inline-flex items-center gap-1 rounded-md bg-[#ef4444] px-2 py-0.5 text-[11px] font-black tracking-wider text-white sm:text-[12px]">
              DONE
            </span>
          )}
          {isNext && (
            <span className="mon-rampup-next-badge inline-flex items-center gap-1 rounded-md border-2 border-[#ef4444] bg-[#ef4444]/10 px-2 py-0.5 text-[11px] font-black tracking-wider text-[#991b1b] sm:text-[12px]">
        ...
        <p
          className={`text-[16px] font-extrabold leading-snug sm:text-[18px] ${
            isDone ? "text-[#991b1b]" : isNext ? "text-[#111]" : "text-[#9ca3af]"
          }`}
        >
          {label}
        </p>
        <p
          className={`mt-1.5 text-[14px] font-medium leading-[1.8] sm:text-[15px] ${
            isDone ? "text-[#991b1b]/70" : isNext ? "text-[#555]" : "text-[#bcbcbc]"
          }`}
        >
          {text}
        </p>
  ...
  return (
    <div className="relative inline-flex items-center justify-center">
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center">
        <span className="text-[38px] font-black tabular-nums text-[#991b1b] sm:text-[46px]">
          {progress}%
        </span>
...
export function MonitoringRampup({ userName, progressPercent }: MonitoringRampupProps) {
  const introText = `${monitoringRampupCopy.intro}\n${monitoringRampupCopy.body1}\n${monitoringRampupCopy.body2}`;
  const introLines = introText.split("\n");
  const outroLines = monitoringRampupCopy.outro.split("\n");
  const phases = monitoringRampupCopy.phases.map((p, i) => ({
    label: p.label,
    text: p.text,
    state: (i <= 3 ? "done" : "next") as "done" | "next" | "upcoming",
  }));
  ...
  return (
    <section className="rampup-page mon-rampup-page w-full min-w-0 max-w-none">
        ...
        <div className="initiation-action-page mb-6 w-full sm:mb-8">
          <div className="flex justify-center overflow-visible px-2">
            <RevealGroup visible={hero.visible} delay={0}>
              <p className="initiation-brief-badge mon-rampup-title-badge m-0 w-full max-w-[min(100%,52rem)] shadow-[6px_6px_0_#111111]">
                Wrap-up
              </p>
            </RevealGroup>
          </div>
        ...
        <div className="mb-8 flex flex-col items-center sm:mb-10">
          ...
          <RevealGroup visible={hero.visible} delay={STAGGER_MS * 2}>
            <p className="mt-5 text-center text-[30px] font-extrabold leading-snug text-[#111] sm:text-[40px]">
              {userName}님, <span className="text-[#ef4444]">네 번째 관문</span>을 돌파했습니다!
            </p>
          </RevealGroup>
      ...
      <div ref={roadmapHeader.ref}>
        ...
        >
          <div className="h-[2px] w-8 bg-[#ef4444] sm:w-12" />
      ...
      <div ref={outro.ref}>
        <RevealGroup visible={outro.visible} delay={0}>
          <div className="mx-auto mt-12 max-w-2xl rounded-2xl border-2 border-[#ef4444] bg-gradient-to-br from-[#fef2f2] to-[#fee2e2] px-6 py-8 text-center shadow-[0_4px_32px_rgba(239,68,68,0.10)] sm:mt-16 sm:px-10 sm:py-10">
            <div className="mb-4 inline-flex h-14 w-14 items-center justify-center rounded-full bg-[#ef4444]/10 sm:h-16 sm:w-16">
              <Rocket className="h-7 w-7 text-[#ef4444] sm:h-8 sm:w-8" />
            </div>
              ...
              {outroLines.map((line, i) => (
                ...
                >
                  {line.includes("[종료 단계]") ? (
                    <>
                      {line.split("[종료 단계]")[0]}
                      <span className="inline-flex items-center gap-1 rounded-md bg-[#ef4444] px-2 py-0.5 font-extrabold text-white">
                        종료 단계
                      </span>
                      {line.split("[종료 단계]")[1]}
                    </>
