...
const INTRO =
  "실제 사례를 기반으로 만든 에피소드들과 선배 PM들의 꿀팁까지, 깊이 와닿으셨나요? 빈틈없는 항해 지도를 그리는 '기획(Planning)' 단계를 훌륭하게 완수하셨습니다. 하지만 지금부터 본격 시작입니다. 현장에서는 예기치 못한 돌발 변수들이 늘 존재하니까요.";
...
const OUTRO =
  "자, 이제 계획을 현실로 바꾸기 위해 팀원들이 업무에 집중하여 아웃풋을 산출해 내는 [실행 단계]로 진입해 봅시다.";
...
export function PlanRampup({ userName }: PlanRampupProps) {
  ...
  return (
    <div className="space-y-6">
      <h2 className="text-lg font-bold text-[#4A4A4A]">Ramp-up</h2>
      <p className="text-sm text-[#6B6B6B] leading-relaxed">&quot;{INTRO}&quot;</p>
      <p className="text-sm text-[#6B6B6B] leading-relaxed">앞으로 당신은 프로젝트의 다음 여정을 통과하게 됩니다.</p>

      <ul className="rounded-xl border border-[#E5E5E5] bg-white p-4 space-y-4">
        {PHASES.map((phase, i) => (
          <li key={i} className="text-sm text-[#6B6B6B]">
            <span className="font-medium text-[#4A4A4A]">• {phase.label}</span>
            <p className="mt-0.5 leading-relaxed">{phase.text}</p>
          </li>
        ))}
      </ul>

      <p className="text-sm text-[#6B6B6B] leading-relaxed font-medium">&quot;{OUTRO}&quot;</p>
    </div>
  );
}
