...
const ALL_CHOICES: Ep7Choice[] = ["A", "B", "C", "D"];
...
export function Ep7PassionResult({ userName: _userName }: Ep7PassionResultProps) {
  const router = useRouter();
  const { episode7Choice, executionActionHours } = useStore();
  const [showOthers, setShowOthers] = useState(false);

  const choice = (episode7Choice ?? null) as Ep7Choice | null;
  const valid = choice !== null && ALL_CHOICES.includes(choice);
  ...
  const selected = getEp7Result(choice, vocHours, refHours);
  const others = ALL_CHOICES.filter((id) => id !== choice);
  const episodeTitle = ep7Scene.title;

  return (
    <section className="relative left-1/2 w-screen max-w-[100vw] -translate-x-1/2 bg-transparent px-5 pb-16 pt-10 font-sans sm:px-8 sm:pb-20 sm:pt-12" aria-label={`${episodeTitle} 결과`}>
      <div className="mx-auto w-full max-w-7xl">
        <div className="initiation-action-page mb-5 w-full sm:mb-6">
          <div className="flex justify-center px-2">
            <p className="ep1-scene-reveal ep1-result-page-stamp initiation-brief-badge m-0 w-full max-w-[min(100%,52rem)] shadow-[6px_6px_0_#111111]" style={{ animationDelay: "0ms" }} role="status" aria-label="의사결정에 따른 결과">의사결정에 따른 결과</p>
          </div>
        </div>
