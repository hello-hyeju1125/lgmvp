...
const EP3_REVEAL_STAGGER_MS = 110;
function ep3RevealDelay(step: number): CSSProperties {
  return { animationDelay: `${step * EP3_REVEAL_STAGGER_MS}ms` };
}
...
/** Ep1 최성민 상무 / Ep2 김·박 말풍선과 동일: 아바타 + 테두리(`.ep1-supervisor-bubble`) — 아바타·말풍선 각각 순차 페이드 */
...
export function Ep3TeamScene({ userName: _userName }: Ep3TeamSceneProps) {
  ...
  /**
   * 순차 페이드 단계 (110ms 간격):
   * 0 제목 → 1 상황 → 2 아바타 → 3 말풍선(1차 대사) → 4 나레이션 박스
   * → (Q 블록) 5 "Q." → 6 질문/안내 문단 → 옵션 카드 4개 …
   * Q 블록이 없으면 5번에 action 전체 → 옵션은 6번부터
   */
  const optionRevealBase = 5 + (showQuestionBlock ? 2 : 1);
    ...
    <section className="ep1-scene-layout w-full min-w-0 max-w-none space-y-10 sm:space-y-12">
      ...
      <div className="space-y-2 px-2 text-center">
        <p
          ...
          style={ep3RevealDelay(1)}
        >
          {renderDialogueBold(ep3Scene.situation)}
        </p>
      </div>
      ...
      {showQuestionBlock ? (
        <div className="space-y-4 px-1 pt-4 text-center !mt-20 sm:!mt-24 mb-[3.75rem] sm:mb-[4.5rem]">
          ...
          <div
            ...
            style={ep3RevealDelay(6)}
          >
            <p>{renderDialogueBold(actionLead)}</p>
            <p>
              <span className="font-bold text-[color:var(--sim-accent)]">{EP3_ACTION_QUESTION}</span>
            </p>
          </div>
        </div>
      ) : (
