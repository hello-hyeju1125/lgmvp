...
const ALL_CHOICES: Ep4Choice[] = ["A", "B", "C", "D", "E"];
...
export function Ep4RoleResult({ userName: _userName }: Ep4RoleResultProps) {
  const router = useRouter();
  const { episode4Choice, initiationActionHours } = useStore();
  const [showOthers, setShowOthers] = useState(false);

  const choice = episode4Choice as Ep4Choice | null;
  const valid = ALL_CHOICES.includes(choice as Ep4Choice);

  if (!valid || !choice) {
    return (
      <section className="relative left-1/2 w-screen max-w-[100vw] -translate-x-1/2 bg-transparent px-5 py-12 text-center sm:px-8" aria-label="E4 결과">
        <p className="text-sm text-[#374151]">선택 정보가 없습니다.</p>
        <button type="button" onClick={() => router.push("/simulation?phase=ep4-scene")} className="mt-4 rounded-xl border-2 border-black bg-[color:var(--sim-accent)] px-5 py-2.5 text-sm font-bold text-[color:var(--sim-accent-cta-text)]">옵션 선택으로 돌아가기</button>
      </section>
    );
  }

  const hours = initiationActionHours["team_profile"] ?? 0;
  const selected = getEp4Result(choice, hours);
  const others = ALL_CHOICES.filter((id) => id !== choice);
  const episodeTitle = ep4Scene.title;
