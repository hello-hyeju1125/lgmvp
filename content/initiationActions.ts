...
export const initiationScreenCopy = {
  scenarioIntro:
    "리더 발령 소식을 듣고 불과 1주일 뒤, 프로젝트 킥오프 회의가 잡혔습니다. 당신은 AI 전문가가 아니며, 심지어 C-level 김혁기 본부장은 당초 예상보다 더 파격적인 목표 달성을 기대한다는 소문이 들립니다. 킥오프 전, 당신은 제한된 시간 동안 무엇에 가장 집중하겠습니까?",
  timeRule:
    "킥오프까지 당신에게 허락된 준비 시간은 총 40시간(5일)입니다. 아래 제시된 '착수 단계 핵심 액션'들을 확인하고, 당신의 전략에 맞춰 시간을 배분해 보십시오. 각 액션에 투입할 시간은 최소 1단계(3시간)부터 최대 5단계(15시간)까지 자유롭게 설정할 수 있습니다. 어떤 액션에 집중하느냐에 따라 PMBOK 기반의 실무 지식을 습득하게 되며, 당신의 선택은 화면 상단의 5가지 상태 지표에 즉각적으로 반영됩니다.",
  pmbokExplanation:
    "PMBOK이란, 미국 프로젝트관리협회(PMI)에서 발행하는 '프로젝트 관리의 글로벌 표준 가이드'(Project Management Body of Knowledge)입니다. 전 세계 수많은 프로젝트의 성공과 실패 사례를 분석하여, 리더가 프로젝트를 성공적으로 이끌기 위해 반드시 알아야 할 지식과 프로세스를 체계적으로 정리한 'PM들의 바이블(정석)'이라고 할 수 있습니다. 본 시뮬레이션에서 액션을 통해 PMBOK 지식을 습득하면, 불확실한 상황에서 더 정확한 의사결정을 내릴 수 있는 리더십 역량이 강화됩니다.",
